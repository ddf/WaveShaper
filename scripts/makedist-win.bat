...
if %1 == 1 (echo Making WaveShaper Windows DEMO VERSION distribution ...) else (echo Making WaveShaper Windows FULL VERSION distribution ...)
...
msbuild WaveShaper.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild WaveShaper.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\WaveShaper.aaxplugin\Contents\Win32\WaveShaper.aaxplugin --out .\build-win\aax\bin\WaveShaper.aaxplugin\Contents\Win32\WaveShaper.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\WaveShaper.aaxplugin\Contents\x64\WaveShaper.aaxplugin --out .\build-win\aax\bin\WaveShaper.aaxplugin\Contents\x64\WaveShaper.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\WaveShaper.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\WaveShaper.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "WaveShaper Installer" ".\installer\WaveShaper Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\WaveShaper Installer.exe" ".\installer\WaveShaper Demo Installer.exe"
REM -del ".\installer\WaveShaper Installer.exe"
REM -)
