...
def  main():
  ...
  installer = "\installer\WaveShaper Installer.exe"
   
  if demo:
    installer = "\installer\WaveShaper Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\WaveShaper manual.pdf" 
  ]
  ...
  ZIPNAME = "WaveShaper-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "WaveShaper-v" + FULLVERSIONSTR + "-win-demo.zip"
